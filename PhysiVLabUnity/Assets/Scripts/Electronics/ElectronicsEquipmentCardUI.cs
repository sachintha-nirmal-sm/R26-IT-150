...
public class ElectronicsEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public ElectronicsEquipmentDefinition Definition { get; private set; }
    private ElectronicsEquipmentSelectionManager manager;

    public void Initialize(ElectronicsEquipmentDefinition def, ElectronicsEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = ElectronicsIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        ElectronicsUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<ElectronicsUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(ElectronicsIconFactory.GetColor(Definition != null ? Definition.type : ElectronicsEquipmentType.Diode), Color.white, 0.45f);
    }
}
