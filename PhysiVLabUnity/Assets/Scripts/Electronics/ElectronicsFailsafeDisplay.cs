...
public class ElectronicsFailsafeDisplay : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        if (ElectronicsUIManager.Instance != null) yield break;
        CreateOverlay();
    }
    ...
    public static void Hide()
    {
        ...
        var leftover = GameObject.Find("Electronics_FailsafeRoot");
    ...
    private static void CreateOverlay()
    {
        ...
        overlay = new GameObject("Electronics_FailsafeRoot");
        ...
        var header = CreateUiPanel(panel.transform, "Header", new Color(0.08f, 0.28f, 0.42f));
        var hrt = header.GetComponent<RectTransform>();
        hrt.anchorMin = new Vector2(0f, 0.82f);
        hrt.anchorMax = Vector2.one;
        hrt.offsetMin = hrt.offsetMax = Vector2.zero;
        CreateUiText(header.transform, "ELECTRONICS", 40, Color.white, TextAnchor.MiddleCenter);

        CreateUiText(panel.transform,
            "FORWARD BIAS AND REVERSE BIAS OF A DIODE\n\nClick anywhere or press START PRACTICAL",
            30, new Color(0.10f, 0.18f, 0.28f), TextAnchor.MiddleCenter);
    ...
    {
        Hide();
        if (ElectronicsPracticalManager.Instance != null)
            ElectronicsPracticalManager.Instance.StartPractical();
        else if (ElectronicsUIManager.Instance != null)
            ElectronicsUIManager.Instance.StartPractical();
    }
