...
public class WorkEnergyEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private WorkEnergyEquipmentSelectionManager manager;

    public void Initialize(EquipmentDefinition def, WorkEnergyEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = WorkEnergyIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        WorkEnergyUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<WorkEnergyUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(WorkEnergyIconFactory.GetColor(Definition != null ? Definition.type : WorkEnergyEquipmentType.Clay), Color.white, 0.4f);
    }
}
