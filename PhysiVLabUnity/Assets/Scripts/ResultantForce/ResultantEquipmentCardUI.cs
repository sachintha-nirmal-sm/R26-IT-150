...
public class ResultantEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public ResultantEquipmentDefinition Definition { get; private set; }
    private ResultantEquipmentSelectionManager manager;

    public void Initialize(ResultantEquipmentDefinition def, ResultantEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = ResultantIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        ResultantUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<ResultantUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(ResultantIconFactory.GetColor(Definition != null ? Definition.type : ResultantEquipmentType.Trolley), Color.white, 0.45f);
    }
}
