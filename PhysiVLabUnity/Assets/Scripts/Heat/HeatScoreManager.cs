...
public class HeatScoreManager : MonoBehaviour
{
    public static HeatScoreManager Instance { get; private set; }

    [SerializeField] private int rawScore;
    [SerializeField] private int maxRawScore = 192;
    ...
    public void ConfigureMaxRaw(int requiredEquipmentCount = 8)
    {
        maxRawScore =
    ...
    public void AddScore(int amount, bool showFeedback = true)
    {
        if (amount <= 0) return;
        rawScore += amount;
        HeatUIManager.Instance?.UpdateScoreDisplay(GetScore());
        if (showFeedback) HeatFeedbackManager.Instance?.ShowCorrect($"+{amount} MARKS");
    }
    ...
    public void SubtractScore(int amount)
    {
        ...
        HeatUIManager.Instance?.UpdateScoreDisplay(GetScore());
        HeatFeedbackManager.Instance?.ShowIncorrect($"-{amount} MARKS");
        HeatExperimentManager.Instance?.RegisterMistake();
    }
    ...
    {
        rawScore = 0;
        HeatUIManager.Instance?.UpdateScoreDisplay(0);
    }

    public int FinalizeScore()
    {
        int finalScore = GetScore();
        HeatUIManager.Instance?.UpdateScoreDisplay(finalScore);
        return finalScore;
    }
}
