...
public class HeatEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public HeatEquipmentDefinition Definition { get; private set; }
    private HeatEquipmentSelectionManager manager;

    public void Initialize(HeatEquipmentDefinition def, HeatEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = HeatIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<HeatUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(HeatIconFactory.GetColor(Definition != null ? Definition.type : HeatEquipmentType.TestTube), Color.white, 0.45f);
    }
}
