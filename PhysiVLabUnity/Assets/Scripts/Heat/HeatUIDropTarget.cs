...
public class HeatUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private Color normalColor = new Color(0.95f, 0.88f, 0.78f, 0.55f);
    ...
    public bool CanAccept(HeatDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<HeatDragDrop2D>() : null;
    ...
    public void AcceptDrop(HeatDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        HeatEquipmentSnapController.Instance?.OnItemDropped(this, item);
        HeatVariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
