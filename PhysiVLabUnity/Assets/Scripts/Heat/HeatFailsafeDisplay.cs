...
public class HeatFailsafeDisplay : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        if (HeatUIManager.Instance != null) yield break;
        CreateOverlay();
    }
    ...
    public static void Hide()
    {
        ...
        var leftover = GameObject.Find("Heat_FailsafeRoot");
    ...
    private static void CreateOverlay()
    {
        ...
        overlay = new GameObject("Heat_FailsafeRoot");
        ...
        var header = CreateUiPanel(panel.transform, "Header", new Color(0.48f, 0.18f, 0.10f));
        var hrt = header.GetComponent<RectTransform>();
        hrt.anchorMin = new Vector2(0f, 0.82f);
        hrt.anchorMax = Vector2.one;
        hrt.offsetMin = hrt.offsetMax = Vector2.zero;
        CreateUiText(header.transform, "HEAT", 42, Color.white, TextAnchor.MiddleCenter);

        CreateUiText(panel.transform, "PRACTICAL\n\nIllustrating expansion of liquids\nFigure 9.22\n\nClick anywhere or press START", 32, new Color(0.22f, 0.12f, 0.08f), TextAnchor.MiddleCenter);
    ...
    {
        Hide();
        if (HeatExperimentManager.Instance != null)
            HeatExperimentManager.Instance.StartPractical();
        else if (HeatUIManager.Instance != null)
            HeatUIManager.Instance.StartPractical();
    }
