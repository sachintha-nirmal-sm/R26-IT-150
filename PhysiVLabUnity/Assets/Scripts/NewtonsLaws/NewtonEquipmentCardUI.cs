...
public class NewtonEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public NewtonEquipmentDefinition Definition { get; private set; }
    private NewtonEquipmentSelectionManager manager;

    public void Initialize(NewtonEquipmentDefinition def, NewtonEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = NewtonsLawsIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        NewtonUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<NewtonUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(NewtonsLawsIconFactory.GetColor(Definition != null ? Definition.type : NewtonEquipmentType.DynamicsTrolley), Color.white, 0.45f);
    }
}
