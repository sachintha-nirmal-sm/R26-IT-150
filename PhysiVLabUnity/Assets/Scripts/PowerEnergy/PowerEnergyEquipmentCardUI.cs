...
public class PowerEnergyEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public PowerEnergyEquipmentDefinition Definition { get; private set; }
    private PowerEnergyEquipmentSelectionManager manager;

    public void Initialize(PowerEnergyEquipmentDefinition def, PowerEnergyEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = PowerEnergyIconFactory.GetSprite(def.type);
        ...
        if (label != null)
        {
            ...
            label.color = new Color(0.10f, 0.16f, 0.26f);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        PowerEnergyUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<PowerEnergyUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(PowerEnergyIconFactory.GetColor(Definition != null ? Definition.type : PowerEnergyEquipmentType.PowerSupply), Color.white, 0.45f);
    }
}
