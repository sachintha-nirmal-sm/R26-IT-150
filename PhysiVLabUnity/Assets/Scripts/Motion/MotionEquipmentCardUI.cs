...
public class MotionEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public MotionEquipmentDefinition Definition { get; private set; }
    private MotionEquipmentSelectionManager manager;

    public void Initialize(MotionEquipmentDefinition def, MotionEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = MotionIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        MotionUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<MotionUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(MotionIconFactory.GetColor(Definition != null ? Definition.type : MotionEquipmentType.ToyCar), Color.white, 0.45f);
    }
}
