...
public class EquilibriumEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public EquilibriumEquipmentDefinition Definition { get; private set; }
    private EquilibriumEquipmentSelectionManager manager;

    public void Initialize(EquilibriumEquipmentDefinition def, EquilibriumEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = EquilibriumIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        EquilibriumUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<EquilibriumUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(EquilibriumIconFactory.GetColor(Definition != null ? Definition.type : EquilibriumEquipmentType.MeterRuler), Color.white, 0.45f);
    }
}
