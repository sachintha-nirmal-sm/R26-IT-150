...
public class EquilibriumUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public bool CanAccept(EquilibriumDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<EquilibriumDragDrop2D>() : null;
    ...
    public void AcceptDrop(EquilibriumDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        EquilibriumEquipmentSnapController.Instance?.OnItemDropped(this, item);
        EquilibriumVariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
