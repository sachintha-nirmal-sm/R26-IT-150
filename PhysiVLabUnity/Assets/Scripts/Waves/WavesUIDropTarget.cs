...
public class WavesUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public bool CanAccept(WavesDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<WavesDragDrop2D>() : null;
    ...
    public void AcceptDrop(WavesDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        WavesEquipmentSnapController.Instance?.OnItemDropped(this, item);
        WavesVariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
