...
public class WavesEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public WavesEquipmentDefinition Definition { get; private set; }
    private WavesEquipmentSelectionManager manager;

    public void Initialize(WavesEquipmentDefinition def, WavesEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = WavesIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<WavesUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(WavesIconFactory.GetColor(Definition != null ? Definition.type : WavesEquipmentType.Slinky), Color.white, 0.45f);
    }
}
