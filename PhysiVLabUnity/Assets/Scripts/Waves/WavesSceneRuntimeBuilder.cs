...
public class WavesSceneRuntimeBuilder : MonoBehaviour
{
    ...
    public void WireReferencesOnPlay(bool showWelcome, bool force)
    {
        ...
        {
            Debug.LogWarning("Waves practical: wiring failed.");
            return;
        }
    ...
    private void Awake()
    {
        if (GetComponent<WavesFailsafeDisplay>() == null)
            gameObject.AddComponent<WavesFailsafeDisplay>();
    ...
    private void BuildInternal()
    {
        ...
        try
        {
            ...
            CreateUI();
            WireReferences(true);
            Debug.Log("Waves practical UI built.");
        }
        catch (System.Exception ex)
        {
            Debug.LogError("Waves practical BUILD FAILED: " + ex.Message + "\n" + ex.StackTrace);
        }
    }
    ...
    private void LoadFont()
    {
        ...
        {
            Debug.LogWarning("Waves practical: font load skipped — " + ex.Message);
            defaultFont = null;
        }
    }
    ...
    private void EnsureEventSystem()
    {
        ...
        {
            Debug.LogWarning("Waves EventSystem: " + ex.Message);
        }
    }
    ...
    private void CreateManagers()
    {
        ...
        AddMgr<WavesExperimentManager>("WavesExperimentManager");
        AddMgr<WavesUIManager>("WavesUIManager");
        AddMgr<WavesScoreManager>("WavesScoreManager");
        AddMgr<WavesFeedbackManager>("WavesFeedbackManager");
        AddMgr<WavesAttemptManager>("WavesAttemptManager");
        AddMgr<WavesSaveManager>("WavesSaveManager");
        AddMgr<WavesProfileManager>("WavesProfileManager");
        AddMgr<WavesEquipmentSelectionManager>("WavesEquipmentSelectionManager");
        AddMgr<WavesEquipmentSnapController>("WavesEquipmentSnapController");
        AddMgr<WavesEquipmentManager>("WavesEquipmentManager");
        AddMgr<WavesAssemblyManager>("WavesAssemblyManager");
        AddMgr<WavesWaveController>("WavesWaveController");
        AddMgr<WavesVisualController>("WavesVisualController");
        AddMgr<WavesObservationTableManager>("WavesObservationTableManager");
        AddMgr<WavesQuestionManager>("WavesQuestionManager");
        AddMgr<WavesConclusionManager>("WavesConclusionManager");
        AddMgr<WavesVariableMatchingManager>("WavesVariableMatchingManager");
        AddMgr<WavesResultManager>("WavesResultManager");
    }
    ...
    private void CreateUI()
    {
        ...
        var title = Text("Title", headerP.transform, "WAVES    Transverse slinky", 28, TextAlignmentOptions.MidlineLeft, Vector2.zero, Vector2.zero, new Vector2(0.02f, 0.50f), new Vector2(0.62f, 1f), new Vector2(0.5f, 0.5f));
        ...
        var diagramTex = Resources.Load<Texture2D>("Waves/diagram");
        if (diagramTex != null)
        {
        ...
        introPanel.AddComponent<WavesIntroClickToStart>();
        ...
        requiredArea.AddComponent<WavesUIDropTarget>().Configure("RequiredEquipment", "Any", Vector2.zero);
        ...
        var refs = canvasObj.AddComponent<WavesUIRefs>();
        refs.UiVersion = 2;
        ...
        refs.ShakeSideBtn = lab.shakeSide; refs.ShakePushBtn = lab.shakePush; refs.ObserveBtn = lab.observe;
        refs.Tray = lab.tray;
        refs.TableVisual = lab.table; refs.SlinkyVisual = lab.slinky; refs.HandVisual = lab.hand; refs.WaveArrow = lab.arrow;
        refs.DirectionLabel = lab.dirLabel; refs.ParticleLabel = lab.partLabel;
    ...
    public bool WireReferences(bool showWelcome)
    {
        ...
        if (refs == null || WavesUIManager.Instance == null) return false;
        WavesFeedbackManager.Instance?.BindUI(refs.FeedbackPanel, refs.FeedbackText, refs.ScoreChangeText, refs.FeedbackGroup);
        WavesUIManager.Instance.BindAll(refs, showWelcome);
        WavesEquipmentSelectionManager.Instance?.SetupUI(refs.CardContainer, refs.RequiredArea, refs.CardPrefab);
        WavesObservationTableManager.Instance?.Bind(refs.DataTableText);
        WavesResultManager.Instance?.Bind(refs.FinalScore, refs.ResultDetails, refs.StatusText);
        WavesEquipmentManager.Instance?.Bind(refs.Tray);
        WavesVariableMatchingManager.Instance?.Bind(refs.VariablePanel != null ? refs.VariablePanel.transform : null);
        WavesConclusionManager.Instance?.BindPhrases(refs.PhraseButtons);
        WavesVisualController.Instance?.Bind(refs.TableVisual, refs.SlinkyVisual, refs.HandVisual, refs.WaveArrow, refs.DirectionLabel, refs.ParticleLabel);
        WavesFailsafeDisplay.Hide();
    ...
    private LabBits BuildLaboratory(Transform parent, Color accent, Color green)
    {
        ...
        Text("ExpTitle", host.transform, "EXPERIMENT AREA  —  slinky on the table, top view", 20, TextAlignmentOptions.MidlineLeft, new Vector2(14, -8), new Vector2(760, 36));

        var tableZone = Panel(host.transform, "TableZone", new Vector2(0.04f, 0.18f), new Vector2(0.96f, 0.88f), Vector2.zero, Vector2.zero, new Color(0.78f, 0.84f, 0.90f, 0.45f));
        tableZone.AddComponent<WavesUIDropTarget>().Configure("TableZone", "Table|Slinky", Vector2.zero);
        var table = Panel(tableZone.transform, "TableVisual", new Vector2(0.02f, 0.08f), new Vector2(0.98f, 0.92f), Vector2.zero, Vector2.zero, new Color(0.72f, 0.52f, 0.30f));
        table.SetActive(false);

        var slinkyZone = Panel(table.transform, "SlinkyZone", new Vector2(0.03f, 0.18f), new Vector2(0.97f, 0.82f), Vector2.zero, Vector2.zero, new Color(0, 0, 0, 0), false);
        slinkyZone.AddComponent<WavesUIDropTarget>().Configure("SlinkyZone", "Slinky|Ribbon0|Ribbon1|Ribbon2|Ribbon3|Ribbon4", Vector2.zero);
        slinkyZone.GetComponent<Image>().raycastTarget = true;

        for (int i = 0; i < 5; i++)
        {
            float cx = 0.20f + i * 0.155f;
            var slot = Panel(slinkyZone.transform, "RibbonSlot" + i, new Vector2(cx - 0.018f, 0.42f), new Vector2(cx + 0.018f, 0.58f), Vector2.zero, Vector2.zero, new Color(0.92f, 0.32f, 0.55f, 0.45f));
            slot.AddComponent<WavesUIDropTarget>().Configure("RibbonSlot" + i, "Ribbon0|Ribbon1|Ribbon2|Ribbon3|Ribbon4", Vector2.zero);
            StretchText("Mark", slot.transform, (i + 1).ToString(), 12, TextAlignmentOptions.Center, new Color(0.45f, 0.08f, 0.22f), 0);
        }

        var slinky = Panel(slinkyZone.transform, "SlinkyVisual", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0, 0, 0, 0), false);
        slinky.SetActive(false);
        slinky.transform.SetAsLastSibling();

        var hand = Panel(host.transform, "HandVisual", new Vector2(0.00f, 0.38f), new Vector2(0.12f, 0.72f), Vector2.zero, Vector2.zero, Color.white);
        hand.GetComponent<Image>().sprite = WavesIconFactory.GetNamed("hand");
        hand.GetComponent<Image>().preserveAspect = true;
        hand.SetActive(false);

        var arrow = Panel(host.transform, "WaveArrow", new Vector2(0.18f, 0.06f), new Vector2(0.82f, 0.16f), Vector2.zero, Vector2.zero, Color.white);
        arrow.GetComponent<Image>().sprite = WavesIconFactory.GetNamed("arrow");
        arrow.GetComponent<Image>().preserveAspect = true;
        arrow.SetActive(false);
        var dirLabel = StretchText("DirLabel", host.transform, "Direction of wave: —", 18, TextAlignmentOptions.MidlineLeft, new Color(0.12f, 0.16f, 0.22f), 8);
        var dirRt = dirLabel.rectTransform;
        dirRt.anchorMin = new Vector2(0.18f, 0.00f);
        dirRt.anchorMax = new Vector2(0.62f, 0.08f);
        var partLabel = StretchText("PartLabel", host.transform, "Ribbon motion: —", 18, TextAlignmentOptions.MidlineRight, new Color(0.12f, 0.16f, 0.22f), 8);
        var pRt = partLabel.rectTransform;
        pRt.anchorMin = new Vector2(0.50f, 0.00f);
        pRt.anchorMax = new Vector2(0.96f, 0.08f);
        ...
        var shakeSide = Btn("ShakeSide", controls.transform, "SHAKE SIDE TO SIDE", accent, 0, 36);
        var shakePush = Btn("ShakePush", controls.transform, "PUSH AND PULL", new Color(0.75f, 0.28f, 0.18f), 0, 36);
        var observe = Btn("Observe", controls.transform, "OBSERVE RIBBONS", new Color(0.45f, 0.32f, 0.72f), 0, 36);

        var adaptive = labPanel.AddComponent<WavesAdaptiveLab>();
        adaptive.Bind(
    ...
    private ConclusionBits BuildConclusionPanel(Transform parent, Color green)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var choice = buttons[i].gameObject.AddComponent<WavesPhraseChoiceButton>();
    ...
    private VariableBits BuildVariablePanel(Transform parent, Color green)
    {
        ...
        MakeVarRow(list.transform, "Shake", "How the slinky is shaken (side to side)");
        MakeVarRow(list.transform, "RibbonMotion", "Motion of the ribbons");
        MakeVarRow(list.transform, "Slinky", "Same slinky used throughout");
        MakeVarRow(list.transform, "Table", "Slinky kept flat on the table");
        MakeVarRow(list.transform, "Ribbons", "Ribbons tied at the same points");
    ...
    private Button MakeChoiceBtn(Transform parent, string name, string label, Color color, string itemId, string zone)
    {
        ...
        var choice = btn.gameObject.AddComponent<WavesVariableChoiceButton>();
    ...
    private GameObject Panel(Transform parent, string name, Vector2 aMin, Vector2 aMax, Vector2 offMin, Vector2 offMax, Color color, bool raycast = true)
    {
        ...
        img.sprite = WavesIconFactory.White();
        img.color = color;
...
#pragma warning restore CS0618
    ...
    private ScrollRect CreateScrollAnchored(Transform parent, Vector2 aMin, Vector2 aMax)
    {
        ...
        content.AddComponent<WavesAdaptiveGrid>();
    ...
    private GameObject CreateCardPrefab()
    {
        ...
        card.AddComponent<WavesEquipmentCardUI>();
    ...
    {
        public GameObject panel, table, slinky, hand, arrow;
        public TextMeshProUGUI live, physics, dirLabel, partLabel;
        public Button confirm, shakeSide, shakePush, observe;
...
public class WavesUIRefs : MonoBehaviour
{
    ...
    public TextMeshProUGUI DirectionLabel, ParticleLabel;
    ...
    public GameObject TableVisual, SlinkyVisual, HandVisual, WaveArrow;
    ...
    public Button ConfirmSetupBtn, ShakeSideBtn, ShakePushBtn, ObserveBtn;
...
public class WavesAdaptiveGrid : MonoBehaviour
{
...
public class WavesAdaptiveLab : MonoBehaviour
{
