...
public class TurningConclusionManager : MonoBehaviour
{
    public static TurningConclusionManager Instance { get; private set; }
    ...
    public void Evaluate()
    {
        ...
        if (ok)
        {
            TurningScoreManager.Instance?.AddScore(10, false);
        ...
        else
        {
            ...
            TurningFeedbackManager.Instance?.ShowMessage(
                "✗ INCORRECT\nRebuild the conclusion: moment = force × perpendicular distance from the pivot.",
                "-5 MARKS",
    ...
    {
        TurningUIManager.Instance?.SetConclusionPreview(CurrentSentence());
    }
    ...
    public void BindPhrases(Button[] buttons)
    {
        ...
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            var choice = buttons[i].GetComponent<TurningPhraseChoiceButton>() ?? buttons[i].gameObject.AddComponent<TurningPhraseChoiceButton>();
