...
public class TurningEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public TurningEquipmentDefinition Definition { get; private set; }
    private TurningEquipmentSelectionManager manager;

    public void Initialize(TurningEquipmentDefinition def, TurningEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = TurningIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        TurningUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<TurningUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(TurningIconFactory.GetColor(Definition != null ? Definition.type : TurningEquipmentType.WoodenStick), Color.white, 0.45f);
    }
}
