...
public class TurningUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public bool CanAccept(TurningDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<TurningDragDrop2D>() : null;
    ...
    public void AcceptDrop(TurningDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        TurningEquipmentSnapController.Instance?.OnItemDropped(this, item);
        TurningVariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
