...
public class ElecEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public ElecEquipmentDefinition Definition { get; private set; }
    private ElecEquipmentSelectionManager manager;

    public void Initialize(ElecEquipmentDefinition def, ElecEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = ElecIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<ElecUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(ElecIconFactory.GetColor(Definition != null ? Definition.type : ElecEquipmentType.Bulb), Color.white, 0.45f);
    }
}
