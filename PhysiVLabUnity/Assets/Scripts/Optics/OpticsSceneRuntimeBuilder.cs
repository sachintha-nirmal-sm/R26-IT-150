...
public class OpticsSceneRuntimeBuilder : MonoBehaviour
{
    ...
    public void WireReferencesOnPlay(bool showWelcome, bool force)
    {
        ...
        {
            Debug.LogWarning("Optics practical: wiring failed.");
            return;
        }
    ...
    private void Awake()
    {
        if (GetComponent<OpticsFailsafeDisplay>() == null)
            gameObject.AddComponent<OpticsFailsafeDisplay>();
    ...
    private void BuildInternal()
    {
        ...
        try
        {
            ...
            CreateUI();
            WireReferences(true);
            Debug.Log("Optics practical UI built.");
        }
        catch (System.Exception ex)
        {
            Debug.LogError("Optics practical BUILD FAILED: " + ex.Message + "\n" + ex.StackTrace);
        }
    }
    ...
    private void LoadFont()
    {
        ...
        {
            Debug.LogWarning("Optics practical: font load skipped — " + ex.Message);
            defaultFont = null;
        }
    }
    ...
    private void EnsureEventSystem()
    {
        ...
        {
            Debug.LogWarning("Optics EventSystem: " + ex.Message);
        }
    }
    ...
    private void CreateManagers()
    {
        ...
        AddMgr<OpticsExperimentManager>("OpticsExperimentManager");
        AddMgr<OpticsUIManager>("OpticsUIManager");
        AddMgr<OpticsScoreManager>("OpticsScoreManager");
        AddMgr<OpticsFeedbackManager>("OpticsFeedbackManager");
        AddMgr<OpticsAttemptManager>("OpticsAttemptManager");
        AddMgr<OpticsSaveManager>("OpticsSaveManager");
        AddMgr<OpticsProfileManager>("OpticsProfileManager");
        AddMgr<OpticsEquipmentSelectionManager>("OpticsEquipmentSelectionManager");
        AddMgr<OpticsEquipmentSnapController>("OpticsEquipmentSnapController");
        AddMgr<OpticsEquipmentManager>("OpticsEquipmentManager");
        AddMgr<OpticsAssemblyManager>("OpticsAssemblyManager");
        AddMgr<OpticsVisualController>("OpticsVisualController");
        AddMgr<OpticsObservationTableManager>("OpticsObservationTableManager");
        AddMgr<OpticsQuestionManager>("OpticsQuestionManager");
        AddMgr<OpticsConclusionManager>("OpticsConclusionManager");
        AddMgr<OpticsVariableMatchingManager>("OpticsVariableMatchingManager");
        AddMgr<OpticsResultManager>("OpticsResultManager");
    }
    ...
    private void CreateUI()
    {
        ...
        var title = Text("Title", headerP.transform, "GEOMETRICAL OPTICS    Concave mirror", 28, TextAlignmentOptions.MidlineLeft, Vector2.zero, Vector2.zero, new Vector2(0.02f, 0.50f), new Vector2(0.62f, 1f), new Vector2(0.5f, 0.5f));
        ...
        var progressBarBg = Panel(headerP.transform, "ProgressBarBg", new Vector2(0.02f, 0.00f), new Vector2(0.98f, 0.08f), Vector2.zero, Vector2.zero, new Color(0.05f, 0.10f, 0.18f, 0.5f));
        ...
        var introText = StretchText("IntroText", introPanel.transform, "", 24, TextAlignmentOptions.TopLeft, new Color(0.14f, 0.18f, 0.24f), 12);
        ...
        var diagramTex = Resources.Load<Texture2D>("Optics/diagram");
        if (diagramTex != null)
        {
        ...
        introPanel.AddComponent<OpticsIntroClickToStart>();
        ...
        requiredArea.AddComponent<OpticsUIDropTarget>().Configure("RequiredEquipment", "Any", Vector2.zero);
        ...
        var refs = canvasObj.AddComponent<OpticsUIRefs>();
        refs.UiVersion = 2;
        ...
        refs.OpenWindowBtn = lab.openWindow; refs.SharpImageBtn = lab.sharp; refs.RecordFBtn = lab.record;
        refs.Tray = lab.tray;
        refs.ClosedWindow = lab.closedWindow; refs.OpenWindow = lab.openWindowVisual;
        refs.OutdoorScene = lab.scene; refs.MirrorVisual = lab.mirror;
        refs.ScreenVisual = lab.screen; refs.RulerVisual = lab.ruler; refs.RaysVisual = lab.rays;
        refs.DistanceLabel = lab.distLabel; refs.ImageStatusLabel = lab.statusLabel;
        refs.DistanceSlider = lab.slider;
    ...
    public bool WireReferences(bool showWelcome)
    {
        ...
        if (refs == null || OpticsUIManager.Instance == null) return false;
        OpticsFeedbackManager.Instance?.BindUI(refs.FeedbackPanel, refs.FeedbackText, refs.ScoreChangeText, refs.FeedbackGroup);
        OpticsUIManager.Instance.BindAll(refs, showWelcome);
        OpticsEquipmentSelectionManager.Instance?.SetupUI(refs.CardContainer, refs.RequiredArea, refs.CardPrefab);
        OpticsObservationTableManager.Instance?.Bind(refs.DataTableText);
        OpticsResultManager.Instance?.Bind(refs.FinalScore, refs.ResultDetails, refs.StatusText);
        OpticsEquipmentManager.Instance?.Bind(refs.Tray);
        OpticsVariableMatchingManager.Instance?.Bind(refs.VariablePanel != null ? refs.VariablePanel.transform : null);
        OpticsConclusionManager.Instance?.BindPhrases(refs.PhraseButtons);
        try
        {
            OpticsVisualController.Instance?.Bind(
                refs.ClosedWindow, refs.OpenWindow, refs.OutdoorScene,
                refs.MirrorVisual, refs.ScreenVisual, refs.RulerVisual, refs.RaysVisual,
                refs.DistanceLabel, refs.ImageStatusLabel, refs.DistanceSlider);
        }
        catch (System.Exception ex)
        {
            Debug.LogWarning("Optics visual bind skipped: " + ex.Message);
        }
        OpticsFailsafeDisplay.Hide();
    ...
    private LabBits BuildLaboratory(Transform parent, Color accent, Color green)
    {
        ...
        StretchText("TrayLabel", trayHeader.transform, "EQUIPMENT\nDrag into the room", 24, TextAlignmentOptions.Center, Color.white, 6).fontStyle = FontStyles.Bold;
        ...
        var host = Panel(labPanel.transform, "ExperimentHost", new Vector2(0.21f, 0.16f), new Vector2(0.735f, 0.99f), Vector2.zero, Vector2.zero, new Color(0.90f, 0.86f, 0.78f));
        host.AddComponent<OpticsUIDropTarget>().Configure("RoomZone", "ConcaveMirror|WhiteScreen|MeterRuler", Vector2.zero);
        Text("ExpTitle", host.transform, "EXPERIMENT AREA  —  room with window, side view", 18, TextAlignmentOptions.MidlineLeft, new Vector2(14, -8), new Vector2(760, 32));

        var wall = Panel(host.transform, "LeftWall", new Vector2(0.00f, 0.12f), new Vector2(0.22f, 0.92f), Vector2.zero, Vector2.zero, new Color(0.78f, 0.70f, 0.58f));
        var closedWindow = Panel(wall.transform, "ClosedWindow", new Vector2(0.12f, 0.18f), new Vector2(0.88f, 0.88f), Vector2.zero, Vector2.zero, Color.white);
        closedWindow.GetComponent<Image>().sprite = OpticsIconFactory.GetNamed("window");
        closedWindow.GetComponent<Image>().preserveAspect = true;
        var openWindowVisual = Panel(wall.transform, "OpenWindow", new Vector2(0.12f, 0.18f), new Vector2(0.88f, 0.88f), Vector2.zero, Vector2.zero, Color.white);
        openWindowVisual.GetComponent<Image>().sprite = OpticsIconFactory.GetNamed("window");
        openWindowVisual.GetComponent<Image>().preserveAspect = true;
        openWindowVisual.SetActive(false);
        var scene = Panel(wall.transform, "OutdoorScene", new Vector2(0.18f, 0.24f), new Vector2(0.82f, 0.82f), Vector2.zero, Vector2.zero, Color.white);
        scene.GetComponent<Image>().sprite = OpticsIconFactory.GetNamed("scene");
        scene.GetComponent<Image>().preserveAspect = true;
        scene.SetActive(false);

        var rays = Panel(host.transform, "Rays", new Vector2(0.20f, 0.38f), new Vector2(0.62f, 0.70f), Vector2.zero, Vector2.zero, Color.white);
        rays.GetComponent<Image>().sprite = OpticsIconFactory.GetNamed("rays");
        rays.GetComponent<Image>().preserveAspect = true;
        rays.GetComponent<Image>().color = new Color(1f, 1f, 1f, 0.85f);
        rays.SetActive(false);

        var mirrorZone = Panel(host.transform, "MirrorZone", new Vector2(0.68f, 0.18f), new Vector2(0.96f, 0.82f), Vector2.zero, Vector2.zero, new Color(0.70f, 0.82f, 0.95f, 0.35f));
        mirrorZone.AddComponent<OpticsUIDropTarget>().Configure("MirrorZone", "ConcaveMirror", Vector2.zero);
        StretchText("MirrorHint", mirrorZone.transform, "Mirror\nhere", 16, TextAlignmentOptions.Center, new Color(0.20f, 0.32f, 0.48f), 8);
        var mirror = Panel(mirrorZone.transform, "MirrorVisual", new Vector2(0.15f, 0.08f), new Vector2(0.92f, 0.92f), Vector2.zero, Vector2.zero, Color.white);
        mirror.GetComponent<Image>().sprite = OpticsIconFactory.GetSprite(OpticsEquipmentType.ConcaveMirror);
        mirror.GetComponent<Image>().preserveAspect = true;
        mirror.SetActive(false);

        var screenZone = Panel(host.transform, "ScreenZone", new Vector2(0.28f, 0.16f), new Vector2(0.66f, 0.84f), Vector2.zero, Vector2.zero, new Color(0.95f, 0.95f, 0.90f, 0.18f));
        screenZone.AddComponent<OpticsUIDropTarget>().Configure("ScreenZone", "WhiteScreen", Vector2.zero);
        var screen = Panel(host.transform, "ScreenVisual", new Vector2(0.42f, 0.18f), new Vector2(0.53f, 0.78f), Vector2.zero, Vector2.zero, Color.white);
        screen.GetComponent<Image>().sprite = OpticsIconFactory.GetSprite(OpticsEquipmentType.WhiteScreen);
        screen.GetComponent<Image>().preserveAspect = true;
        screen.SetActive(false);
        var sceneOnScreen = Panel(screen.transform, "SceneImage", new Vector2(0.18f, 0.18f), new Vector2(0.82f, 0.82f), Vector2.zero, Vector2.zero, Color.white);
        sceneOnScreen.GetComponent<Image>().sprite = OpticsIconFactory.GetNamed("scene");
        sceneOnScreen.GetComponent<Image>().preserveAspect = true;
        sceneOnScreen.GetComponent<Image>().raycastTarget = false;

        var rulerZone = Panel(host.transform, "RulerZone", new Vector2(0.24f, 0.04f), new Vector2(0.92f, 0.16f), Vector2.zero, Vector2.zero, new Color(0.92f, 0.86f, 0.55f, 0.25f));
        rulerZone.AddComponent<OpticsUIDropTarget>().Configure("RulerZone", "MeterRuler", Vector2.zero);
        var ruler = Panel(rulerZone.transform, "RulerVisual", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, Color.white);
        ruler.GetComponent<Image>().sprite = OpticsIconFactory.GetSprite(OpticsEquipmentType.MeterRuler);
        ruler.GetComponent<Image>().preserveAspect = true;
        ruler.SetActive(false);

        var distLabel = StretchText("DistLabel", host.transform, "Mirror–screen distance:  —", 16, TextAlignmentOptions.MidlineLeft, new Color(0.12f, 0.16f, 0.22f), 4);
        var distRt = distLabel.rectTransform;
        distRt.anchorMin = new Vector2(0.24f, 0.88f);
        distRt.anchorMax = new Vector2(0.70f, 0.98f);
        var statusLabel = StretchText("StatusLabel", host.transform, "Image on screen:  —", 16, TextAlignmentOptions.MidlineRight, new Color(0.12f, 0.16f, 0.22f), 4);
        var stRt = statusLabel.rectTransform;
        stRt.anchorMin = new Vector2(0.48f, 0.88f);
        stRt.anchorMax = new Vector2(0.98f, 0.98f);

        var sliderHost = Panel(host.transform, "SliderHost", new Vector2(0.24f, 0.00f), new Vector2(0.96f, 0.05f), Vector2.zero, Vector2.zero, new Color(0, 0, 0, 0), false);
        var slider = sliderHost.AddComponent<Slider>();
        slider.minValue = 8f;
        slider.maxValue = 36f;
        slider.value = 12f;
        var sliderBg = Panel(sliderHost.transform, "Background", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0.70f, 0.74f, 0.80f));
        var fillArea = Panel(sliderHost.transform, "Fill Area", new Vector2(0.02f, 0.25f), new Vector2(0.98f, 0.75f), Vector2.zero, Vector2.zero, new Color(0, 0, 0, 0), false);
        var fill = Panel(fillArea.transform, "Fill", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0.18f, 0.48f, 0.72f));
        var handleSlide = Panel(sliderHost.transform, "Handle Slide Area", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0, 0, 0, 0), false);
        var handle = Panel(handleSlide.transform, "Handle", new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), Vector2.zero, new Vector2(22, 28), new Color(0.12f, 0.32f, 0.55f));
        slider.fillRect = fill.GetComponent<RectTransform>();
        slider.handleRect = handle.GetComponent<RectTransform>();
        slider.targetGraphic = handle.GetComponent<Image>();

        var side = Panel(labPanel.transform, "SidePanel", new Vector2(0.745f, 0.16f), new Vector2(0.992f, 0.99f), Vector2.zero, Vector2.zero, new Color(0.95f, 0.97f, 1f));
        var live = StretchText("LiveReadings", side.transform, "", 20, TextAlignmentOptions.TopLeft, new Color(0.12f, 0.16f, 0.22f), 10);
        ...
        var physics = StretchText("PhysicsText", side.transform, "", 18, TextAlignmentOptions.TopLeft, new Color(0.18f, 0.22f, 0.30f), 10);
        ...
        var controls = Panel(labPanel.transform, "LabControls", new Vector2(0.21f, 0.01f), new Vector2(0.735f, 0.15f), Vector2.zero, Vector2.zero, new Color(0.10f, 0.18f, 0.30f));
        ...
        var openWindow = Btn("OpenWindow", controls.transform, "OPEN WINDOW", accent, 0, 36);
        var confirm = Btn("ConfirmSetup", controls.transform, "CONFIRM SETUP", green, 0, 36);
        var sharp = Btn("SharpImage", controls.transform, "IMAGE IS SHARP", new Color(0.45f, 0.32f, 0.72f), 0, 36);
        var record = Btn("RecordF", controls.transform, "RECORD f", new Color(0.75f, 0.45f, 0.12f), 0, 36);

        var adaptive = labPanel.AddComponent<OpticsAdaptiveLab>();
        adaptive.Bind(
        ...
        return new LabBits
        {
            ...
            openWindowVisual = openWindowVisual,
            scene = scene,
            mirror = mirror,
            screen = screen,
            ruler = ruler,
    ...
    private ConclusionBits BuildConclusionPanel(Transform parent, Color green)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var choice = buttons[i].gameObject.AddComponent<OpticsPhraseChoiceButton>();
    ...
    private VariableBits BuildVariablePanel(Transform parent, Color green)
    {
        ...
        MakeVarRow(list.transform, "ScreenDistance", "Distance between mirror and screen");
        MakeVarRow(list.transform, "ImageSharpness", "How clear the image is on the screen");
        MakeVarRow(list.transform, "Mirror", "Same concave mirror used throughout");
        MakeVarRow(list.transform, "DistantObject", "Outdoor scene kept far away");
        MakeVarRow(list.transform, "Screen", "Same white screen used to catch the image");
    ...
    private Button MakeChoiceBtn(Transform parent, string name, string label, Color color, string itemId, string zone)
    {
        ...
        var choice = btn.gameObject.AddComponent<OpticsVariableChoiceButton>();
    ...
    private GameObject Panel(Transform parent, string name, Vector2 aMin, Vector2 aMax, Vector2 offMin, Vector2 offMax, Color color, bool raycast = true)
    {
        ...
        img.sprite = OpticsIconFactory.White();
        img.color = color;
...
#pragma warning restore CS0618
    ...
    private ScrollRect CreateScrollAnchored(Transform parent, Vector2 aMin, Vector2 aMax)
    {
        ...
        content.AddComponent<OpticsAdaptiveGrid>();
    ...
    private GameObject CreateCardPrefab()
    {
        ...
        card.AddComponent<OpticsEquipmentCardUI>();
    ...
    {
        public GameObject panel, closedWindow, openWindowVisual, scene, mirror, screen, ruler, rays;
        public TextMeshProUGUI live, physics, distLabel, statusLabel;
        public Button confirm, openWindow, sharp, record;
...
public class OpticsUIRefs : MonoBehaviour
{
    ...
    public TextMeshProUGUI DistanceLabel, ImageStatusLabel;
    ...
    public GameObject ClosedWindow, OpenWindow, OutdoorScene, MirrorVisual, ScreenVisual, RulerVisual, RaysVisual;
    ...
    public Button ConfirmSetupBtn, OpenWindowBtn, SharpImageBtn, RecordFBtn;
...
public class OpticsAdaptiveGrid : MonoBehaviour
{
...
public class OpticsAdaptiveLab : MonoBehaviour
{
