...
public class OpticsEquipmentCardUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public OpticsEquipmentDefinition Definition { get; private set; }
    private OpticsEquipmentSelectionManager manager;

    public void Initialize(OpticsEquipmentDefinition def, OpticsEquipmentSelectionManager mgr)
    {
        Definition = def;
        manager = mgr;
        EnsureLayout();
        ...
        if (iconImage != null)
        {
            iconImage.sprite = OpticsIconFactory.GetSprite(def.type);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        OpticsUIDropTarget target = null;
        if (EventSystem.current != null)
        {
            ...
            {
                target = hit.gameObject.GetComponentInParent<OpticsUIDropTarget>();
    ...
    private void EnsureLayout()
    {
        ...
        if (bg != null)
            bg.color = Color.Lerp(OpticsIconFactory.GetColor(Definition != null ? Definition.type : OpticsEquipmentType.ConcaveMirror), Color.white, 0.45f);
    }
}
