...
public class OpticsUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public bool CanAccept(OpticsDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<OpticsDragDrop2D>() : null;
    ...
    public void AcceptDrop(OpticsDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        OpticsEquipmentSnapController.Instance?.OnItemDropped(this, item);
        OpticsVariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
