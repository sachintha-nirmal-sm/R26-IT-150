...
public class OpticsScoreManager : MonoBehaviour
{
    public static OpticsScoreManager Instance { get; private set; }
    ...
    public void AddScore(int amount, bool showFeedback = true)
    {
        if (amount <= 0) return;
        rawScore += amount;
        OpticsUIManager.Instance?.UpdateScoreDisplay(GetScore());
        if (showFeedback) OpticsFeedbackManager.Instance?.ShowCorrect($"+{amount} MARKS");
    }
    ...
    public void SubtractScore(int amount)
    {
        ...
        OpticsUIManager.Instance?.UpdateScoreDisplay(GetScore());
        OpticsFeedbackManager.Instance?.ShowIncorrect($"-{amount} MARKS");
        OpticsExperimentManager.Instance?.RegisterMistake();
    }
    ...
    {
        rawScore = 0;
        OpticsUIManager.Instance?.UpdateScoreDisplay(0);
    }

    public int FinalizeScore()
    {
        int finalScore = GetScore();
        OpticsUIManager.Instance?.UpdateScoreDisplay(finalScore);
        return finalScore;
    }
}
