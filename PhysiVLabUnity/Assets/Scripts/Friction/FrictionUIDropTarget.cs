...
public class FrictionUIDropTarget : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public bool CanAccept(FrictionDragDrop2D item)
    {
    ...
    {
        var item = eventData.pointerDrag != null ? eventData.pointerDrag.GetComponent<FrictionDragDrop2D>() : null;
    ...
    public void AcceptDrop(FrictionDragDrop2D item)
    {
        item.SnapTo(transform, snapPosition);
        if (background != null) background.color = normalColor;
        FrictionEquipmentSnapController.Instance?.OnItemDropped(this, item);
        VariableMatchingManager.Instance?.OnItemDropped(this, item);
    }
}
