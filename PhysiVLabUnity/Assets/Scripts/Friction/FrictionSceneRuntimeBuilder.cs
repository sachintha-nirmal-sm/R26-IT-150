...
public class FrictionSceneRuntimeBuilder : MonoBehaviour
{
    ...
    public void WireReferencesOnPlay(bool showWelcome, bool force)
    {
        ...
        {
            Debug.LogWarning("Friction: wiring failed.");
            return;
        }
    ...
    private void Awake()
    {
        if (GetComponent<FrictionFailsafeDisplay>() == null)
            gameObject.AddComponent<FrictionFailsafeDisplay>();
    ...
    private void BuildInternal()
    {
        ...
        try
        {
            ...
            CreateUI();
            WireReferences(true);
            Debug.Log("Friction practical UI built.");
        }
        catch (System.Exception ex)
        {
            Debug.LogError("Friction BUILD FAILED: " + ex.Message + "\n" + ex.StackTrace);
        }
    }
    ...
    private void LoadFont()
    {
        ...
        {
            Debug.LogWarning("Friction: font load skipped — " + ex.Message);
            defaultFont = null;
        }
    }
    ...
    private void EnsureEventSystem()
    {
        ...
        {
            Debug.LogWarning("Friction EventSystem: " + ex.Message);
        }
    }
    ...
    private void CreateManagers()
    {
        ...
        AddMgr<FrictionExperimentManager>("FrictionExperimentManager");
        AddMgr<FrictionUIManager>("FrictionUIManager");
        AddMgr<FrictionScoreManager>("FrictionScoreManager");
        AddMgr<FrictionFeedbackManager>("FrictionFeedbackManager");
        AddMgr<FrictionAttemptManager>("FrictionAttemptManager");
        AddMgr<FrictionSaveManager>("FrictionSaveManager");
        AddMgr<FrictionProfileManager>("FrictionProfileManager");
        AddMgr<FrictionEquipmentSelectionManager>("FrictionEquipmentSelectionManager");
        AddMgr<FrictionEquipmentSnapController>("FrictionEquipmentSnapController");
        AddMgr<FrictionEquipmentManager>("FrictionEquipmentManager");
        AddMgr<WoodenBlockController>("WoodenBlockController");
        AddMgr<BlockSurfaceController>("BlockSurfaceController");
        AddMgr<SandpaperController>("SandpaperController");
        AddMgr<NewtonBalanceController>("NewtonBalanceController");
        AddMgr<NewtonBalancePointer>("NewtonBalancePointer");
        AddMgr<FrictionAppliedForceController>("FrictionAppliedForceController");
        AddMgr<FrictionForceController>("FrictionForceController");
        AddMgr<PullController>("PullController");
        AddMgr<LimitingFrictionDetector>("LimitingFrictionDetector");
        AddMgr<FrictionTrialManager>("FrictionTrialManager");
        AddMgr<FrictionMeasurementManager>("FrictionMeasurementManager");
        AddMgr<FrictionObservationTableManager>("FrictionObservationTableManager");
        AddMgr<FrictionGraphController>("FrictionGraphController");
        AddMgr<FrictionQuestionManager>("FrictionQuestionManager");
        AddMgr<ConclusionManager>("ConclusionManager");
        AddMgr<VariableMatchingManager>("VariableMatchingManager");
        AddMgr<FrictionResultManager>("FrictionResultManager");
    }
    ...
    {
        Color header = new Color(0.28f, 0.18f, 0.10f);
        Color accent = new Color(0.72f, 0.42f, 0.16f);
        ...
        Panel(canvasObj.transform, "ScreenBg", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0.94f, 0.95f, 0.97f));

        var headerP = Panel(canvasObj.transform, "Header", new Vector2(0, 0.91f), Vector2.one, Vector2.zero, Vector2.zero, header);
        var title = Text("Title", headerP.transform, "FRICTION    Activity: Surface Area Investigation", 28, TextAlignmentOptions.MidlineLeft, new Vector2(16, -6), new Vector2(1200, 44));
        ...
        progress.color = new Color(1f, 0.90f, 0.75f);
        ...
        stepLabel.color = new Color(1f, 0.90f, 0.75f);
        ...
        attempts.color = new Color(1f, 0.90f, 0.75f);
        attempts.enableAutoSizing = true;
        attempts.fontSizeMin = 12;
        attempts.fontSizeMax = 20;
        var progressBarBg = Panel(headerP.transform, "ProgressBarBg", new Vector2(0.30f, 0.06f), new Vector2(0.70f, 0.18f), Vector2.zero, Vector2.zero, new Color(0.12f, 0.08f, 0.05f, 0.5f));
        var progressBarFill = Panel(progressBarBg.transform, "Fill", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0.85f, 0.62f, 0.22f));
        ...
        var bottom = Panel(canvasObj.transform, "BottomBar", Vector2.zero, new Vector2(1, 0.11f), Vector2.zero, Vector2.zero, new Color(0.22f, 0.16f, 0.12f));
        ...
        introPanel.AddComponent<FrictionIntroClickToStart>();
        ...
        equipHint.color = new Color(0.32f, 0.22f, 0.12f);
        ...
        var requiredAreaHost = Panel(equipPanel.transform, "RequiredArea", new Vector2(0, 0), new Vector2(1, 0.24f), new Vector2(8, 6), new Vector2(-8, -4), new Color(1f, 0.95f, 0.88f));
        var reqLabel = StretchText("ReqLabel", requiredAreaHost.transform, "REQUIRED EQUIPMENT", 16, TextAlignmentOptions.MidlineLeft, new Color(0.32f, 0.20f, 0.10f), 6);
        ...
        requiredArea.AddComponent<FrictionUIDropTarget>().Configure("RequiredEquipment", "Any", Vector2.zero);
        ...
        var dataTablePanel = MakeTextPanel(main.transform, "DataTablePanel", "TableTitle", "TABLE 5.4", "DataTableText");
        ...
        var refs = canvasObj.AddComponent<FrictionUIRefs>();
        refs.UiVersion = 4;
        ...
        refs.PullBtn = lab.pull; refs.StopPullBtn = lab.stop; refs.ResetRunBtn = lab.resetRun;
        refs.RecordBtn = lab.record; refs.ConfirmSetupBtn = lab.confirm;
        refs.SurfaceA = lab.sa; refs.SurfaceB = lab.sb; refs.SurfaceC = lab.sc;
        refs.ForceArrow = lab.forceArrow; refs.FrictionArrow = lab.frictionArrow;
        refs.BlockVisual = lab.block; refs.SandpaperVisual = lab.sandpaper;
        refs.BalanceVisual = lab.balance; refs.Pointer = lab.pointer;
        refs.ScaleFill = lab.scaleFill; refs.ReadingLabel = lab.reading;
        refs.SurfaceLabel = lab.surfaceLabel; refs.AreaLabel = lab.areaLabel;
        refs.RoughnessLabel = lab.roughness; refs.Tray = lab.tray;
    ...
    private bool WireReferences(bool showWelcome)
    {
        ...
        if (refs == null || FrictionUIManager.Instance == null) return false;
        FrictionFeedbackManager.Instance?.BindUI(refs.FeedbackPanel, refs.FeedbackText, refs.ScoreChangeText, refs.FeedbackGroup);
        FrictionUIManager.Instance.BindAll(refs, showWelcome);
        FrictionEquipmentSelectionManager.Instance?.SetupUI(refs.CardContainer, refs.RequiredArea, refs.CardPrefab);
        FrictionObservationTableManager.Instance?.Bind(refs.DataTableText);
        FrictionGraphController.Instance?.Bind(refs.AreaGraph, refs.ForceGraph, refs.DotPrefab);
        FrictionResultManager.Instance?.Bind(refs.FinalScore, refs.ResultDetails, refs.StatusText);
        WoodenBlockController.Instance?.Bind(refs.BlockVisual);
        BlockSurfaceController.Instance?.Bind(refs.SurfaceLabel, refs.AreaLabel);
        SandpaperController.Instance?.Bind(refs.SandpaperVisual, refs.RoughnessLabel);
        NewtonBalanceController.Instance?.Bind(refs.BalanceVisual, refs.ReadingLabel, refs.ScaleFill);
        NewtonBalancePointer.Instance?.Bind(refs.Pointer);
        FrictionEquipmentManager.Instance?.Bind(refs.Tray);
        VariableMatchingManager.Instance?.Bind(refs.VariablePanel != null ? refs.VariablePanel.transform : null);
        ConclusionManager.Instance?.BindPhrases(refs.PhraseButtons);
        FrictionFailsafeDisplay.Hide();
    ...
    private LabBits BuildLaboratory(Transform parent, Color accent, Color green)
    {
        ...
        var tray = Panel(labPanel.transform, "EquipmentTray", new Vector2(0.008f, 0.16f), new Vector2(0.20f, 0.99f), Vector2.zero, Vector2.zero, new Color(0.95f, 0.93f, 0.88f, 1f));
        var trayHeader = Panel(tray.transform, "TrayHeader", new Vector2(0f, 0.90f), Vector2.one, Vector2.zero, Vector2.zero, new Color(0.32f, 0.20f, 0.10f));
        ...
        var scrollObj = Panel(tray.transform, "Scroll", new Vector2(0.04f, 0.02f), new Vector2(0.96f, 0.88f), Vector2.zero, Vector2.zero, new Color(0.95f, 0.93f, 0.88f, 0.2f));
        ...
        var host = Panel(labPanel.transform, "ExperimentHost", new Vector2(0.21f, 0.16f), new Vector2(0.735f, 0.99f), Vector2.zero, Vector2.zero, new Color(0.88f, 0.91f, 0.95f));
        Text("ExpTitle", host.transform, "EXPERIMENT AREA", 28, TextAlignmentOptions.MidlineLeft, new Vector2(14, -8), new Vector2(520, 36));

        var table = Panel(host.transform, "Table", new Vector2(0.04f, 0.08f), new Vector2(0.96f, 0.38f), Vector2.zero, Vector2.zero, new Color(0.55f, 0.40f, 0.24f));
        table.AddComponent<FrictionUIDropTarget>().Configure("Table", "Sandpaper|WoodenBlock", Vector2.zero);
        StretchText("TableLabel", table.transform, "TABLE", 28, TextAlignmentOptions.Bottom, Color.white, 6);

        var sandZone = Panel(host.transform, "SandpaperZone", new Vector2(0.18f, 0.30f), new Vector2(0.72f, 0.46f), Vector2.zero, Vector2.zero, new Color(0.78f, 0.62f, 0.38f, 0.35f));
        sandZone.name = "SandpaperZone";
        sandZone.AddComponent<FrictionUIDropTarget>().Configure("SandpaperZone", "Sandpaper|WoodenBlock", Vector2.zero);
        var sandpaper = Panel(sandZone.transform, "SandpaperVisual", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(0.78f, 0.62f, 0.38f));
        sandpaper.GetComponent<Image>().sprite = FrictionIconFactory.GetNamed("sandpaper");
        sandpaper.GetComponent<Image>().preserveAspect = false;
        sandpaper.SetActive(false);

        var blockZone = Panel(host.transform, "BlockZone", new Vector2(0.28f, 0.40f), new Vector2(0.62f, 0.68f), Vector2.zero, Vector2.zero, new Color(0.72f, 0.48f, 0.22f, 0.25f));
        blockZone.name = "BlockZone";
        blockZone.AddComponent<FrictionUIDropTarget>().Configure("Block", "WoodenBlock", Vector2.zero);
        var block = Panel(blockZone.transform, "WoodenBlock", new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), Vector2.zero, new Vector2(220, 90), Color.white);
        block.GetComponent<Image>().sprite = FrictionIconFactory.GetNamed("block");
        block.GetComponent<Image>().preserveAspect = true;
        var blockRt = block.GetComponent<RectTransform>();
        blockRt.pivot = new Vector2(0.5f, 0.5f);

        var hook = Panel(host.transform, "HookZone", new Vector2(0.62f, 0.48f), new Vector2(0.92f, 0.78f), Vector2.zero, Vector2.zero, new Color(0.55f, 0.62f, 0.72f, 0.25f));
        hook.name = "HookZone";
        hook.AddComponent<FrictionUIDropTarget>().Configure("BlockHook", "NewtonBalance", Vector2.zero);
        var balance = Panel(hook.transform, "NewtonBalance", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, Color.white);
        balance.GetComponent<Image>().sprite = FrictionIconFactory.GetNamed("balance");
        balance.GetComponent<Image>().preserveAspect = true;
        balance.SetActive(false);
        var pointer = Panel(balance.transform, "Pointer", new Vector2(0.15f, 0.55f), new Vector2(0.15f, 0.55f), Vector2.zero, new Vector2(16, 70), new Color(0.85f, 0.18f, 0.12f));
        var scaleBg = Panel(balance.transform, "ScaleBg", new Vector2(0.55f, 0.15f), new Vector2(0.92f, 0.85f), Vector2.zero, Vector2.zero, new Color(0.92f, 0.94f, 0.96f));
        ...
        fill.fillAmount = 0f;
        var reading = StretchText("Reading", balance.transform, "0.0 N", 26, TextAlignmentOptions.Top, new Color(0.12f, 0.16f, 0.22f), 4);

        var forceArrow = Panel(host.transform, "ForceArrow", new Vector2(0.62f, 0.82f), new Vector2(0.62f, 0.82f), Vector2.zero, new Vector2(120, 28), new Color(0.18f, 0.48f, 0.82f));
        forceArrow.GetComponent<RectTransform>().pivot = new Vector2(0f, 0.5f);
        StretchText("FTxt", forceArrow.transform, "Applied Force →", 18, TextAlignmentOptions.Center, Color.white, 2);
        var frictionArrow = Panel(host.transform, "FrictionArrow", new Vector2(0.38f, 0.82f), new Vector2(0.38f, 0.82f), Vector2.zero, new Vector2(120, 28), new Color(0.78f, 0.22f, 0.18f));
        frictionArrow.GetComponent<RectTransform>().pivot = new Vector2(1f, 0.5f);
        StretchText("RTxt", frictionArrow.transform, "← Friction", 18, TextAlignmentOptions.Center, Color.white, 2);

        var surfaceLabel = Text("SurfaceLabel", host.transform, "Surface A\n30 cm × 20 cm", 24, TextAlignmentOptions.TopLeft, new Vector2(12, -44), new Vector2(340, 70));
        var areaLabel = Text("AreaLabel", host.transform, "Area = 600 cm²\nWeight = 60 N", 24, TextAlignmentOptions.TopLeft, new Vector2(12, -118), new Vector2(340, 70));
        ...
        var live = StretchText("LiveReadings", side.transform, "Applied Force: 0.0 N", 26, TextAlignmentOptions.TopLeft, new Color(0.12f, 0.16f, 0.22f), 10);
        ...
        physics.fontSizeMax = 20;
        var pRt = physics.rectTransform;
        pRt.anchorMin = new Vector2(0.06f, 0.02f);
        pRt.anchorMax = new Vector2(0.94f, 0.42f);
        var roughness = Text("Roughness", side.transform, "Surface roughness:\nCONSTANT", 20, TextAlignmentOptions.Center, Vector2.zero, new Vector2(240, 52), new Vector2(0.5f, 0.46f), new Vector2(0.5f, 0.46f), new Vector2(0.5f, 0.5f));

        var actionRow = Panel(labPanel.transform, "ActionRow", new Vector2(0.21f, 0.01f), new Vector2(0.992f, 0.15f), Vector2.zero, Vector2.zero, new Color(0.22f, 0.16f, 0.12f));
        ...
        var sa = Btn("SurfaceA", actionRow.transform, "A 600 cm²", accent, 0, 36);
        var sb = Btn("SurfaceB", actionRow.transform, "B 300 cm²", accent, 0, 36);
        var sc = Btn("SurfaceC", actionRow.transform, "C 200 cm²", accent, 0, 36);
        var confirm = Btn("Confirm", actionRow.transform, "CONFIRM SETUP", green, 0, 36);
        var pull = Btn("Pull", actionRow.transform, "HOLD TO PULL", new Color(0.15f, 0.48f, 0.78f), 0, 36);
        var stop = Btn("Stop", actionRow.transform, "STOP", new Color(0.75f, 0.25f, 0.2f), 0, 36);
        ...
        var adaptive = labPanel.AddComponent<FrictionAdaptiveLab>();
        adaptive.Bind(
        ...
        return new LabBits
        {
            ...
            sb = sb,
            sc = sc,
            forceArrow = forceArrow.GetComponent<RectTransform>(),
            frictionArrow = frictionArrow.GetComponent<RectTransform>(),
            block = block.GetComponent<RectTransform>(),
            sandpaper = sandpaper.GetComponent<RectTransform>(),
            balance = balance.GetComponent<RectTransform>(),
    ...
    private GraphBits BuildGraphPanel(Transform parent, Color accent)
    {
        ...
        panel.SetActive(false);
        StretchText("GTitle", panel.transform, "CONTACT AREA vs LIMITING FRICTION     •     APPLIED FORCE vs TIME", 22, TextAlignmentOptions.Top, new Color(0.18f, 0.14f, 0.10f), 8).fontStyle = FontStyles.Bold;
    ...
    private ConclusionBits BuildConclusionPanel(Transform parent, Color green)
    {
        ...
        var preview = Text("Preview", panel.transform, "", 22, TextAlignmentOptions.Center, Vector2.zero, new Vector2(1400, 70), new Vector2(0.5f, 0.38f), new Vector2(0.5f, 0.38f), new Vector2(0.5f, 0.5f));
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var choice = buttons[i].gameObject.AddComponent<PhraseChoiceButton>();
    ...
    private VariableBits BuildVariablePanel(Transform parent, Color green)
    {
        ...
        var list = Panel(panel.transform, "Rows", new Vector2(0.03f, 0.16f), new Vector2(0.97f, 0.86f), Vector2.zero, Vector2.zero, new Color(0.97f, 0.96f, 0.93f));
        ...
        MakeVarRow(list.transform, "ContactArea", "Contact area");
        MakeVarRow(list.transform, "LimitingFriction", "Limiting frictional force");
        MakeVarRow(list.transform, "Weight", "Weight");
        MakeVarRow(list.transform, "Roughness", "Sandpaper roughness");
        MakeVarRow(list.transform, "WoodenBlock", "Wooden block");
        MakeVarRow(list.transform, "SurfaceMaterial", "Surface material");
        var cont = BigBtn("VariableContinue", panel.transform, "NEXT STEP", new Vector2(0.5f, 0.06f), Vector2.zero, new Vector2(420, 78), green);
        return new VariableBits { panel = panel, independent = list, dependent = list, controlled = list, continueBtn = cont };
    }
    ...
    private void MakeVarRow(Transform parent, string id, string label)
    {
        ...
        row.AddComponent<Image>().color = new Color(1f, 0.98f, 0.94f);
    ...
    private Button MakeChoiceBtn(Transform parent, string name, string label, Color color, string itemId, string zone)
    {
        ...
        var choice = btn.gameObject.AddComponent<VariableChoiceButton>();
...
#pragma warning disable CS0618
        tmp.enableWordWrapping = name == "Instruction" || name.Contains("Objective") || name.Contains("Intro") || name.Contains("Surface") || name.Contains("Area");
...
#pragma warning restore CS0618
    ...
    private Button ChoiceBtn(string name, Transform parent, string letter, string text)
    {
        ...
        var letterBg = Panel(obj.transform, "Letter", new Vector2(0f, 0.12f), new Vector2(0f, 0.88f), new Vector2(10, 0), new Vector2(68, 0), new Color(0.72f, 0.42f, 0.16f), false);
    ...
    {
        var scrollObj = Panel(parent, "Scroll", aMin, aMax, Vector2.zero, Vector2.zero, new Color(0.97f, 0.96f, 0.93f));
        var scroll = scrollObj.AddComponent<ScrollRect>();
        var viewport = Panel(scrollObj.transform, "Viewport", Vector2.zero, Vector2.one, Vector2.zero, Vector2.zero, new Color(1, 1, 1, 0.01f));
        viewport.AddComponent<Mask>().showMaskGraphic = false;
        ...
        content.AddComponent<FrictionAdaptiveGrid>();
        scroll.viewport = viewport.GetComponent<RectTransform>();
    ...
    private GameObject CreateCardPrefab()
    {
        ...
        card.AddComponent<Image>().color = new Color(0.96f, 0.93f, 0.86f);
        card.AddComponent<FrictionEquipmentCardUI>();
    ...
    private struct LabBits
    {
        ...
        public TextMeshProUGUI live, physics, reading, surfaceLabel, areaLabel, roughness;
        public Button pull, stop, resetRun, record, confirm, sa, sb, sc;
        public RectTransform forceArrow, frictionArrow, block, sandpaper, balance, pointer;
        public Image scaleFill;
    ...
    {
        public GameObject panel, independent, dependent, controlled;
        public Button continueBtn;
    }
...
public class FrictionUIRefs : MonoBehaviour
{
    ...
    public TextMeshProUGUI ReadingLabel, SurfaceLabel, AreaLabel, RoughnessLabel;
    public Image ProgressBar, ScaleFill;
    ...
    public RectTransform AreaGraph, ForceGraph, ForceArrow, FrictionArrow, BlockVisual, SandpaperVisual, BalanceVisual, Pointer;
    public Button StartBtn, Next, Reset, Retry, ResetYes, ResetNo, RetryYes, RetryNo, ViewProfileBtn, ViewResultsBtn;
    public Button PullBtn, StopPullBtn, ResetRunBtn, RecordBtn, ConfirmSetupBtn, SurfaceA, SurfaceB, SurfaceC;
...
public class FrictionAdaptiveGrid : MonoBehaviour
{
...
public class FrictionAdaptiveLab : MonoBehaviour
{
