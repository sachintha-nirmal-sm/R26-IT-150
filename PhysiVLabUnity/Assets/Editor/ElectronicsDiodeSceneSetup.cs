...
/// <summary>
/// Creates Assets/Scenes/ElectronicsDiodeExperiment.unity.
/// Menu: Tools → PhysiVLab → Create Electronics Scene
/// </summary>
[InitializeOnLoad]
public static class ElectronicsDiodeSceneSetup
{
    private const string ScenePath = "Assets/Scenes/ElectronicsDiodeExperiment.unity";
    ...
    static ElectronicsDiodeSceneSetup()
    {
        EditorApplication.delayCall += () => EnsureBuildSettings(false);
    }

    [MenuItem("Tools/PhysiVLab/Create Electronics Scene")]
    public static void CreateElectronicsSceneMenu()
    {
        CreateOrRepair(true);
    }
    ...
    public static void CreateOrRepair(bool interactive)
    {
        ...
        if (interactive)
        {
            ...
            StripOtherLabs();
            var root = Object.FindAnyObjectByType<ElectronicsSceneRuntimeBuilder>();
            if (root == null)
            {
                var lab = new GameObject("ElectronicsLab");
                lab.AddComponent<ElectronicsSceneRuntimeBuilder>();
                lab.AddComponent<ElectronicsRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
            else if (root.GetComponent<ElectronicsRuntimeBootstrap>() == null)
            {
                root.gameObject.AddComponent<ElectronicsRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
        }
