...
/// <summary>
/// Creates Assets/Scenes/GeometricalOpticsExperiment.unity.
/// Menu: Tools → PhysiVLab → Create Optics Scene
/// </summary>
[InitializeOnLoad]
public static class GeometricalOpticsSceneSetup
{
    private const string ScenePath = "Assets/Scenes/GeometricalOpticsExperiment.unity";
    ...
    static GeometricalOpticsSceneSetup()
    {
        EditorApplication.delayCall += () => EnsureBuildSettings(false);
    }

    [MenuItem("Tools/PhysiVLab/Create Optics Scene")]
    public static void CreateOpticsSceneMenu()
    {
        CreateOrRepair(true);
    }
    ...
    public static void CreateOrRepair(bool interactive)
    {
        ...
        if (interactive)
        {
            ...
            StripOtherLabs();
            var root = Object.FindAnyObjectByType<OpticsSceneRuntimeBuilder>();
            if (root == null)
            {
                var lab = new GameObject("OpticsLab");
                lab.AddComponent<OpticsSceneRuntimeBuilder>();
                lab.AddComponent<OpticsRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
            else if (root.GetComponent<OpticsRuntimeBootstrap>() == null)
            {
                root.gameObject.AddComponent<OpticsRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
        }
