...
/// <summary>
/// Creates Assets/Scenes/HeatExpansionExperiment.unity.
/// Menu: Tools → PhysiVLab → Create Heat Scene
/// </summary>
[InitializeOnLoad]
public static class HeatSceneSetup
{
    private const string ScenePath = "Assets/Scenes/HeatExpansionExperiment.unity";
    ...
    static HeatSceneSetup()
    {
        EditorApplication.delayCall += () => EnsureBuildSettings(false);
    }

    [MenuItem("Tools/PhysiVLab/Create Heat Scene")]
    public static void CreateHeatSceneMenu()
    {
        CreateOrRepair(true);
    }
    ...
    public static void CreateOrRepair(bool interactive)
    {
        ...
        if (interactive)
        {
            ...
            StripOtherLabs();
            var root = Object.FindAnyObjectByType<HeatSceneRuntimeBuilder>();
            if (root == null)
            {
                var lab = new GameObject("HeatLab");
                lab.AddComponent<HeatSceneRuntimeBuilder>();
                lab.AddComponent<HeatRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
            else if (root.GetComponent<HeatRuntimeBootstrap>() == null)
            {
                root.gameObject.AddComponent<HeatRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
        }
