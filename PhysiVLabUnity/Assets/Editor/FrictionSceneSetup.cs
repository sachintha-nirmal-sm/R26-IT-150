...
/// <summary>
/// Creates Assets/Scenes/FrictionExperiment.unity.
/// Menu: Tools → PhysiVLab → Create Friction Scene
/// </summary>
[InitializeOnLoad]
public static class FrictionSceneSetup
{
    private const string ScenePath = "Assets/Scenes/FrictionExperiment.unity";
    ...
    static FrictionSceneSetup()
    {
        EditorApplication.delayCall += () => EnsureBuildSettings(false);
    }

    [MenuItem("Tools/PhysiVLab/Create Friction Scene")]
    public static void CreateFrictionSceneMenu()
    {
        CreateOrRepair(true);
    }
    ...
    public static void CreateOrRepair(bool interactive)
    {
        ...
        if (interactive)
        {
            ...
            StripOtherLabs();
            var root = Object.FindAnyObjectByType<FrictionSceneRuntimeBuilder>();
            if (root == null)
            {
                var lab = new GameObject("FrictionLab");
                lab.AddComponent<FrictionSceneRuntimeBuilder>();
                lab.AddComponent<FrictionRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
            else if (root.GetComponent<FrictionRuntimeBootstrap>() == null)
            {
                root.gameObject.AddComponent<FrictionRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
        }
