...
/// <summary>
/// Creates Assets/Scenes/PowerEnergyAppliancesExperiment.unity.
/// Menu: Tools → PhysiVLab → Create Power Energy Appliances Scene
/// </summary>
[InitializeOnLoad]
public static class PowerEnergyAppliancesSceneSetup
{
    private const string ScenePath = "Assets/Scenes/PowerEnergyAppliancesExperiment.unity";
    ...
    static PowerEnergyAppliancesSceneSetup()
    {
        EditorApplication.delayCall += () => EnsureBuildSettings(false);
    }

    [MenuItem("Tools/PhysiVLab/Create Power Energy Appliances Scene")]
    public static void CreatePowerEnergySceneMenu()
    {
        CreateOrRepair(true);
    }
    ...
    public static void CreateOrRepair(bool interactive)
    {
        ...
        if (interactive)
        {
            ...
            StripOtherLabs();
            var root = Object.FindAnyObjectByType<PowerEnergySceneRuntimeBuilder>();
            if (root == null)
            {
                var lab = new GameObject("PowerEnergyLab");
                lab.AddComponent<PowerEnergySceneRuntimeBuilder>();
                lab.AddComponent<PowerEnergyRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
            else if (root.GetComponent<PowerEnergyRuntimeBootstrap>() == null)
            {
                root.gameObject.AddComponent<PowerEnergyRuntimeBootstrap>();
                EditorSceneManager.MarkSceneDirty(opened);
                EditorSceneManager.SaveScene(opened);
            }
        }
        ...
        if (interactive)
        {
            EditorUtility.DisplayDialog(
                "PhysiVLab",
                "Power and Energy of Electric Appliances scene is ready:\n" + ScenePath + "\n\nPress Play in Unity to try the lab.",
                "OK");
        }
    }
