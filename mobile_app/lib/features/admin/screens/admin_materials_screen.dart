...
class _AdminMaterialsScreenState extends State<AdminMaterialsScreen>
  ...
  Future<_PickedFile?> _pickFile() async {
    ...
    return _PickedFile(name: file.name, bytes: bytes, size: file.size);
  }
  ...
  Future<List<Map<String, dynamic>>> _fetchLessonsForGrade(String grade) async {
    ...
    try {
      ...
      // Backend not running (e.g. ERR_CONNECTION_REFUSED) — will fallback to Firestore
    }
    ...
    await showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          // Kick off lesson fetch once dialog is open — uses setDialogState to rebuild
          if (loadingLessons) {
            _fetchLessonsForGrade(grade).then((result) {
              lessons = result;
              loadingLessons = false;
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(10)),
                        hintText: 'Brief note about this material…',
                    ...
                    if (uploading) ...[
                      const SizedBox(height: 16),
                      const LinearProgressIndicator(color: _primaryBlue),
                      const SizedBox(height: 6),
                ...
                onPressed: uploading
                    ...
                    : () async {
                        ...
                        if (selectedLessonId == null) {
                          setDialogState(() =>
                              uploadError = 'Please select a lesson first.');
                          return;
                        }
                        if (pickedFile == null) {
                          setDialogState(
  ...
  Future<void> _deleteMaterial(LessonMaterial m) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${m.materialName}"?'),
        actions: [
          TextButton(
...
class _GradeMaterialsTab extends StatefulWidget {
  ...
  @override
  State<_GradeMaterialsTab> createState() => _GradeMaterialsTabState();
}
...
class _GradeMaterialsTabState extends State<_GradeMaterialsTab>
  ...
  Widget _buildBody(AsyncSnapshot<List<LessonMaterial>> snap) {
